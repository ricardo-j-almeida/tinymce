...
const isValidTarget = (schema: Schema, node: Node) => {
  if (NodeType.isText(node)) {
    ...
    return !isBlockElement(schema.getBlockElements(), node) && !Bookmarks.isBookmarkNode(node) && !TransparentElements.isTransparentBlock(schema, node) && !Namespace.isNonHtmlElementRoot(node);
  } else {
    return false;
  }
};
